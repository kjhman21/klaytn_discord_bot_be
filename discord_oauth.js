...
app.post('/ft', async (req, res) => {
	...
	try
	{
		...
		const balanceInPeb = await kip7.balanceOf(address);;
		const decimals = await kip7.decimals();
		var balanceWithDecimal = parseFloat(balance.div(caver.utils.BigNumber(10).pow(18)));

		var role = GetHolderClass(balanceWithDecimal);
		...
		if(role) {
			...
			console.log(`role is not added because the balance is not in the range. balance=${balanceWithDecimal} KLAY, address=${address}`)
